    ...
    for i, account in enumerate(args.accounts):
        ...
        t = Thread(target=search_worker_thread,
                   ...
                   args=(args, account, search_items_queue, stop_bit, pause_bit,
                         ...
                         db_updates_queue, wh_queue, banned_accounts_queue, point))
        t.daemon = True
        t.start()
...
def search_worker_thread(args, account, search_items_queue, stop_bit, pause_bit, encryption_lib_path, status, dbq, whq, baq, point):
        ...
        try:
            ...
            while not stop_bit.is_set():
                ...
                if status['fail'] >= args.max_failures:
                    status['message'] = 'Worker {} failed more than {} scans; possibly banned account. Stopping thread and notifying command.'.format(account['username'], args.max_failures)
                    log.error(status['message'])
                    banned_account = (account['username'], account['password'])
                    baq.put((point, account))
                    stop_bit.set()
